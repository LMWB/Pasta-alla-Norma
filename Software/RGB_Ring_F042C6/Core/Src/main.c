...
pixel_t myPixel = {.R = 0, .B = 0, .G = 0, .F_C = 128};
...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIMER_COUNTER_INSTANCE) {
		/* on STM32 C0xx and F0xx the DMA is stopping it self after Pulse is finished
		 * no need for stopping it manually just trigger a new pulse as soon
		 * the RGB-LEd color has changed
		 * but on C0xx calling the STOP_DMA function will lock it and DMA can't
		 * get started again */
